class Settings {
  ...
  updateAll () {
    ...
    return Promise.all(
      ...
      eachRepository(childPlugins, this.github, this.config, this.log)
    )
...
function eachRepository (childPlugins, github, config, log) {
  ...
  github.paginate('GET /installation/repositories').then(repositories => {
    ...
    return repositories.forEach(repository => {
      ...
      if (config.restrictedRepos.includes(repository.name)) {
        log.debug(`Skipping retricted repo ${repository.name}`)
        return
      } else {
        log.debug(`${repository.name} not in restricted repos ${config.restrictedRepos}`)
      }
