  ...
  return Object.keys(changes).map(plugin => {
    ...
    Object.keys(changes[plugin]).forEach(repo => {
      changes[plugin][repo].forEach(action => {
        rows.push(...compactRowsForAction(plugin, repo, action))
      })
    })
    ...
    const impactSummary = `${repoCount} ${pluralize(repoCount, 'repo', 'repos')}, ${rows.length} ${pluralize(rows.length, targetSingular, targetPlural)} changed`
    return {
      plugin,
      rows,
      repoCount,
...
function compactRowsForAction (plugin, repo, action) {
  if (typeof action === 'string') {
    return [createChangeRow(plugin, repo, plugin, action)]
  }
  ...
  additions.forEach(entry => {
    ...
    rows.push(createChangeRow(plugin, repo, target, summarizeAddedOrDeleted('Added', entry, target)))
  })
  ...
  modifications.forEach((entry, index) => {
    ...
    rows.push(createChangeRow(plugin, repo, target, summarizeModification(match.entry, entry, target)))
  })

  deletions.forEach((entry, index) => {
    if (usedDeletions.has(index)) return
    const target = getChangeTarget(entry, plugin)
    rows.push(createChangeRow(plugin, repo, target, summarizeAddedOrDeleted('Deleted', entry, target)))
  })

  if (rows.length === 0 && action && action.msg) {
    return [createChangeRow(plugin, repo, plugin, action.msg)]
  }

  return rows
}

function createChangeRow (plugin, repo, target, summary) {
  return {
    plugin,
    repo,
    target,
    summary,
    repoCell: markdownTableCell(repo),
    targetCell: markdownTableCell(target),
    summaryCell: markdownTableCell(summary)
  }
}
...
function getChangeIdentity (entry) {
  ...
  return `${field}:${formatValue(entry[field])}`
}

function getChangeTarget (entry, fallback) {
  if (!entry || typeof entry !== 'object' || Array.isArray(entry)) return formatValue(entry) || fallback
  const field = MergeDeep.NAME_FIELDS.find(field => Object.prototype.hasOwnProperty.call(entry, field))
  return field ? formatValue(entry[field]) : fallback
}

function summarizeAddedOrDeleted (verb, entry, target) {
  const summary = summarizeEntry(entry, target)
  return summary ? `${verb}: ${summary}` : verb
}

function summarizeModification (oldEntry, newEntry, target) {
  const diff = summarizeDiff(oldEntry, newEntry)
  if (diff) return diff

  const summary = summarizeEntry(newEntry, target)
  return summary ? `Changed: ${summary}` : 'Changed'
}

function summarizeDiff (oldEntry, newEntry) {
  if (!oldEntry || !newEntry || typeof oldEntry !== 'object' || typeof newEntry !== 'object') return null

  const oldPaths = flattenForSummary(oldEntry, true)
  const newPaths = flattenForSummary(newEntry, true)
  const changed = Object.keys(oldPaths)
    .filter(path => Object.prototype.hasOwnProperty.call(newPaths, path))
    .filter(path => oldPaths[path] !== newPaths[path])
    .slice(0, 3)
    .map(path => `${path}: ${oldPaths[path]} -> ${newPaths[path]}`)

  return changed.length > 0 ? changed.join('; ') : null
}

function summarizeEntry (entry, target) {
  ...
  const parts = Object.keys(flattened)
    .filter(path => flattened[path] !== target)
    .slice(0, 3)
    .map(path => `${path}: ${flattened[path]}`)
...
class Settings {
  ...
  async handleResults () {
    ...
    */
    ...
    stats.changeSections = buildChangeSections(stats.changes)
    stats.reposAffected = new Set(stats.changeSections.flatMap(section => section.rows.map(row => row.repo))).size
    ...
    if (env.CREATE_PR_COMMENT === 'true') {
      ...
      const pluginSectionList = stats.changeSections.map(section => {
        const rows = section.rows.map(row =>
          `| ${row.repoCell} | ${row.targetCell} | ${row.summaryCell} |`
        ).join('\n')
        return `<details>\n<summary>🔌 <strong>${escapeHtml(section.plugin)}</strong> — ${escapeHtml(section.impactSummary)}</summary>\n\n| Repo | Policy / Setting | Change |\n| --- | --- | --- |\n${rows}\n\n</details>`
      })
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        const safeBody = body.length > COMMENT_LIMIT
          ? `${body.substring(0, COMMENT_LIMIT)}... (too many changes to report)`
          : body
