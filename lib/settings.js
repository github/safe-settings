...
class Settings {
  ...
  async handleResults () {
    ...
    */
    ...
    const commentmessage = await eta.render('./summary', stats)
...
${this.results.reduce((x, y) => {
  ...
  if (y.endpoint) {
    return `${x}
<hr>
  ✅ ${y.plugin} plugin will perform \`${y.action}\` on ${y.repo} 
<details>
  <summary>
  Details:
  </summary>
  Will call ${y.endpoint} using payload: 
...
`
      ...
      //const summary = eta.render('./summary', stats)
  ...
  async updateOrg () {
    ...
    if (rulesetsConfig) {
      const RulesetsPlugin = Settings.ORG_PLUGINS.rulesets
      return new RulesetsPlugin(this.nop, this.github, this.repo, rulesetsConfig, this.log).sync().then(res => {
  ...
  async updateRepos (repo) {
    ...
    if (repoConfig) {
      try {
        ...
        const RepoPlugin = Settings.REPO_PLUGINS.repository
  ...
  childPluginsList (repo) {
    ...
    for (const [section, config] of Object.entries(overrideConfig)) {
      ...
      if (section !== 'repositories' && section !== 'repository') {
        ...
        if (section in Settings.REPO_PLUGINS) {
          this.log.debug(`Found section ${section} in the config. Creating plugin...`)
          const Plugin = Settings.REPO_PLUGINS[section]
