...
class Settings {
  ...
  async handleResults () {
    ...
    */
    ...
    if (env.ENABLE_PR_COMMENT !== 'false') {
      const summary = `
...
${this.results.reduce((x, y) => {
  ...
  if (y.endpoint) {
    ...
    if (y.type === 'ERROR') {
      error = true
      return `${x}
| ${y.plugin} | ${y.repo} | ${y.action.msg} | ${JSON.stringify(y.action.additions)} | ${JSON.stringify(y.action.deletions)} | ${JSON.stringify(y.action.modifications)} `
     } else {
...
}, '| Plugin| Repo | Action | Additions | Deletions | Modifications \n| --- | --- | --- | -- | -- | -- ')}
`
  ...
  async updateOrg () {
    ...
    if (rulesetsConfig) {
      ...
      return new RulesetsPlugin(this.nop, this.github, this.repo, rulesetsConfig, this.log, SCOPE.ORG).sync().then(res => {
