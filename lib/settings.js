...
class Settings {
  ...
  static async sync (nop, context, repo, config, ref) {
    ...
    // this.repoConfigs = this.repoConfigs || await this.getRepoConfig(repo)
    ...
    this.results.forEach(res => {
      if (res) {
        ...
        if (res.type === 'ERROR') {
          ...
          stats.changes[res.plugin][res.repo] = true
        }
      }
    })
...
*/
  ...
  async updateRepos (repo) {
    ...
    this.repoConfigs = this.repoConfigs || await this.getRepoConfigs()
