...
`
  ...
  async updateRepos (repo, archived) {
    ...
    // Archived repos get skipped further down, but only after
    // archivePlugin.getState() has spent a repos.get on each one. When the caller
    // already knows — eachRepositoryRepos gets `archived` for free from
    // GET /installation/repositories — skip before issuing any request. On orgs
    // where most repos are archived this is where the bulk of a full sync's
    // rate-limit budget goes.
    //
    ...
    if (archived === true && archivePlugin.getDesiredArchiveState() !== false) {
    ...
    if (repoConfig) {
      ...
      // This branch has no repoConfig, so getState() was never reached and the
      // isArchived guard above did not run — child plugins would still issue
      // forbidden writes against an archived repo. Only verify when the archived
      // state is not already known: `false` from the listing needs no request,
      // and `true` already returned above unless an unarchive was requested.
      if (archived !== false) {
        const { isArchived, shouldUnarchive } = await archivePlugin.getState()
        if (isArchived && !shouldUnarchive) {
          this.log.debug(`Skipping child plugin updates for archived repo ${repo.repo}`)
          return
        }
      }
  ...
  async checkAndProcessRepo (owner, name, archived) {
    ...
    // `archived` travels as its own argument, never merged into the repo ref: the
    // Repository plugin does Object.assign({}, settings, repo) and later
    // repos.update(settings), so an `archived` key on the ref would land in the
    // update payload — and would re-archive a repo in the middle of unarchiving
    // it. Plugins must keep receiving a bare { owner, repo }.
