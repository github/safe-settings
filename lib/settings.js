...
`
  ...
  async updateOrg () {
    ...
    if (rulesetsConfig) {
      ...
      if (!RulesetsPlugin) {
        // TomTom: rulesets plugin is intentionally disabled (see Settings.PLUGINS below).
        // Skip silently if the org config still references rulesets.
        this.log.info('Rulesets plugin is disabled in this deployment; skipping org-level rulesets config.')
        return
      }
...
Settings.PLUGINS = {
  ...
  // ^ Disabled in TomTom deployment: rulesets are managed by the separate
  // 'github-as-code' (TypeScript) app in tomtom-internal/github-settings.
  // Re-enable after that app is retired (see docs/rebase-2.1.18/PLAN.md §8).
  // Upstream's rulesets plugin is stable; this is a TomTom deployment policy,
