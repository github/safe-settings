  ...
  return Object.keys(changes).map(plugin => {
    ...
    Object.keys(changes[plugin]).forEach(repo => {
      ...
      changes[plugin][repo].forEach(action => {
        expandedTargetsForAction(plugin, action).forEach(target => {
          if (!targetMap.has(target.target)) {
            targetMap.set(target.target, {
              target: target.target,
              targetCell: markdownTableCell(target.target),
              rows: []
            })
          }
...
function buildAffectedRepoMatrix (changeSections) {
  const repoPlugins = new Map()
  changeSections.forEach(section => {
    section.repoSections.forEach(repoSection => {
      if (!repoPlugins.has(repoSection.repo)) repoPlugins.set(repoSection.repo, new Set())
      repoPlugins.get(repoSection.repo).add(section.plugin)
    })
  })

  const plugins = changeSections.map(section => section.plugin)
  const rows = Array.from(repoPlugins.keys()).sort().map(repo => {
    return `| ${markdownTableCell(repo)} | ${plugins.map(plugin => repoPlugins.get(repo).has(plugin) ? ':hand:' : '').join(' | ')} |`
  })

  return `<details>\n<summary>Overview — ${rows.length} affected ${pluralize(rows.length, 'repo', 'repos')}</summary>\n\nOnly affected repositories are listed.\n\n| Repo | ${plugins.map(plugin => `${markdownTableCell(plugin)} settings`).join(' | ')} |\n| --- | ${plugins.map(() => '---').join(' | ')} |\n${rows.join('\n')}\n\n:hand: -> Changes to be applied.\n\n</details>`
}
  ...
  return changeSections.map(section => {
    const repoBlocks = section.repoSections.map(repoSection => {
      const targetBlocks = repoSection.targets.map(target => {
        return `#### ${markdownTableCell(target.target)}\n\n${renderFieldChangeTable(target.rows)}`
      })
      return `### ${markdownTableCell(repoSection.repo)}\n\n${targetBlocks.join('\n\n')}`
    })
...
function renderFieldChangeTable (rows) {
  const tableRows = rows.map(row => {
    return `<tr><td>${row.changeCell}</td><td><code>${row.fieldCell}</code></td><td>${row.beforeCell}</td><td>${row.afterCell}</td></tr>`
  }).join('\n')

  return `<table><tr><th>Change</th><th>Field</th><th>Before</th><th>After</th></tr>\n${tableRows}\n</table>`
}

function expandedTargetsForAction (plugin, action) {
...
function getChangeTarget (entry, fallback) {
  ...
  const field = MergeDeep.NAME_FIELDS.find(field => Object.prototype.hasOwnProperty.call(entry, field))
  return field ? formatValue(entry[field]).text : fallback
}

function rowsForAddedOrDeleted (change, entry, target) {
  const flattened = flattenForSummary(entry, true)
  const fields = Object.keys(flattened).filter(path => flattened[path].text !== target)
...
function createFieldChangeRow (change, field, before, after) {
  return {
    change,
    field,
    changeCell: markdownTableCell(change),
    fieldCell: markdownTableCell(field),
    beforeCell: renderValueCell(before),
    afterCell: renderValueCell(after)
  }
}
...
function renderValueCell (value) {
  const displayValue = formatValue(value)
  if (!displayValue.details) return markdownTableCell(displayValue)
  return `<details><summary>${markdownTableCell(displayValue.text)}</summary><pre><code>${escapeHtml(displayValue.details)}</code></pre></details>`
}
...
class Settings {
  ...
  async handleResults () {
    ...
    */
    ...
    stats.reposAffected = new Set(stats.changeSections.flatMap(section => section.repoSections.map(repoSection => repoSection.repo))).size
    stats.changeOverview = stats.changeSections.length > 0 ? buildAffectedRepoMatrix(stats.changeSections) : ''
    ...
    stats.checkRunDetails = stats.changeDetails.length > 50000
      ? `${stats.changeOverview}\n\nDetailed field-level changes are available in the pull request comment.`
      : stats.changeDetails
    ...
    if (env.CREATE_PR_COMMENT === 'true') {
      ...
      const allSections = stats.changeSections.length === 0
        ...
        : [stats.changeOverview, ...pluginSectionList, errorSection]
