...
${this.results.reduce((x, y) => {
  ...
  if (y.endpoint) {
    ...
    if (y.type === 'ERROR') {
      error = true
      return `${x}
| ${y.plugin} | ${y.repo} | ${JSON.stringify(y.action.additions)} | ${JSON.stringify(y.action.deletions)} | ${JSON.stringify(y.action.modifications)} `
     } else {
...
}, '| Plugin| Repo | Additions | Deletions | Modifications \n| --- | --- | -- | -- | -- ')}
`
