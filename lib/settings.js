...
function isEmptyChange (action) {
  ...
  const isEmpty = (v) => v === null || v === undefined || (typeof v === 'object' && Object.keys(v).length === 0)
  return isEmpty(additions) && isEmpty(deletions) && isEmpty(modifications)
}
  ...
  async handleResults () {
    ...
    */
    ...
    if (env.CREATE_PR_COMMENT === 'true') {
      ...
      const pluginSectionList = Object.keys(stats.changes).map(plugin => {
        const repos = Object.keys(stats.changes[plugin])
        const rows = repos.map(repo =>
          stats.changes[plugin][repo].map(action => {
            if (typeof action === 'string') {
              return `<tr><td><code>${repo}</code></td><td colspan="3">${action}</td></tr>`
            }
            return `<tr><td><code>${repo}</code></td><td>${prettify(action.additions)}</td><td>${prettify(action.deletions)}</td><td>${prettify(action.modifications)}</td></tr>`
          }).join('\n')
        ).join('\n')
        return `<details>\n<summary>🔌 <strong>${plugin}</strong> — ${repos.length} repo(s) with pending changes</summary>\n\n<table><thead><tr><th>Repo</th><th>Additions</th><th>Deletions</th><th>Modifications</th></tr></thead><tbody>\n${rows}\n</tbody></table>\n\n</details>`
      })
      ...
      const allSections = Object.keys(stats.changes).length === 0
