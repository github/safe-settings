...
module.exports = class CodeScanning extends ErrorStash {
  ...
  validateLanguages (currentConfig, desiredConfig) {
    ...
    // GitHub API returns javascript, typescript, and javascript-typescript as separate entries
    // when the language is detected, but we can only configure javascript-typescript
    // We need to normalize these for validation
    const normalizeLanguageForValidation = (lang) => {
    ...
    // Normalize current languages for validation
    // This converts javascript/typescript to javascript-typescript and removes duplicates
    const normalizedCurrentForValidation = [...new Set(
      normalizedCurrent.map(normalizeLanguageForValidation)
    )]

    // Find unauthorized languages (either not in allowed list or in blocked list)
    const unauthorizedLanguages = []

    for (const lang of normalizedCurrentForValidation) {
  ...
  async enforceAllowedLanguages (currentConfig, validation, resArray) {
    ...
    // Normalize current languages to the format used for validation
    const normalizedCurrentForApi = [...new Set(
      validation.currentLanguages.map(normalizeLanguageForValidation)
    )]

    // Calculate the languages that should be enabled
    // Start with normalized current languages minus unauthorized ones
    const languagesToKeep = normalizedCurrentForApi.filter(
