          ...
          .map(async (branch) => {
            ...
            if (this.isEmpty(branch.protection)) {
              if (branch.name === 'default') {
                ...
                this.log(`Deleting ${this.repo.owner}/${this.repo.repo} default branch protection for branch ${result.data.default_branch}`)
              }
              if (this.nop) {
                resArray.concat([
              ...
              if (branch.name === 'default') {
                const result = await this.github.repos.get(this.repo)
                params = Object.assign(this.repo, { branch: result.data.default_branch })
                this.log(`Setting ${this.repo.owner}/${this.repo.repo} default branch protection for branch ${result.data.default_branch}`)
              }
              ...
              this.log(`Adding ${this.repo.owner}/${this.repo.repo} branch protection ${JSON.stringify(params)}`)
              if (this.nop) {
                resArray.push(
              ...
              return this.github.repos.updateBranchProtection(params).then(res => this.log(`${this.repo.owner}/${this.repo.repo} Branch protection applied successfully ${JSON.stringify(res.url)}`)).catch(e => {this.log(`Error applying branch protection ${JSON.stringify(e)}`); return []})      
            }
          })
      )
    })
