        ...
        this.branches
          ...
          .map(branch => {
            ...
            if (this.isEmpty(branch.protection)) {
              ...
              console.log(`xxx ${JSON.stringify(branch.protection)}`)
              // Hack to handle closures and keep params from changing
              const params = Object.assign({}, p)
              return this.github.repos.getBranchProtection(params).then((result) => {
                ...
                if (changes.additions.length === 0 && changes.modifications.length === 0) {
                  ...
                  if (this.nop) {
                    // resArray.push(
                    //   new NopCommand(this.constructor.name, this.repo, null, `There are no changes for branch ${JSON.stringify(params)}. Skipping branch protection changes`)
                    // )
                    return Promise.resolve(resArray)
                  }
