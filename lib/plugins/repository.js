...
module.exports = class Repository {
    ...
    return this.github.repos.get(this.repo)
      ...
      .then(res => {
        ...
        return this.updaterepo().then(res => {
          this.log(`Successfully updated ${this.repo.owner}/${this.repo.repo}`)
      ...
      .catch(e => {
        if (e.status === 404) {
          if (this.force_create) {
            if (this.template) {
              this.log(`Creating ${this.repo.owner}/${this.repo.repo} using template ${this.template}`)
  ...
  renameBranch (oldname, newname) {
    ...
    this.log.debug(`Rename ${this.settings.owner}/${this.settings.repo} default branch repo with settings ${JSON.stringify(parms)}`)
    if (this.nop) {
      return Promise.resolve([
  ...
  updaterepo () {
    ...
    this.log.debug(`Updating ${this.settings.owner}/${this.settings.repo} with settings ${JSON.stringify(this.topics)} ${JSON.stringify(this.settings)}`)
    if (this.nop) {
      const result = [
  ...
  updatetopics(parms, repoData) {
    if (this.topics) {
      this.log(`Updating ${this.settings.owner}/${this.settings.repo} with topics `, this.topics)
