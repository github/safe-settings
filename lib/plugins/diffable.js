  ...
  sync () {
    ...
    if (this.entries) {
      ...
      return this.find().then(existingRecords => {
        ...
        if (this.nop) {
          resArray.push(new NopCommand(this.constructor.name, this.repo, null, `error calling find for ${this.constructor.name} for repo: ${JSON.stringify(this.repo)} entries ${JSON.stringify(this.entries)} ${e}`, 'ERROR'))
          return Promise.resolve(resArray)
        }
      })
    }
