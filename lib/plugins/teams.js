    ...
    return this.github.teams.getByName({ org: this.repo.owner, team_slug: attrs.name }).then(res => {
      ...
      if (e.status === 404) {
        ...
        return this.github.teams.create(createParam).then(res => {
          ...
          if (e.status === 403) {
            this.logError(`Error creating team "${attrs.name}": Insufficient permissions. Either create the team manually or grant the app "Members: Read & write" permission.`, e)
          } else {
