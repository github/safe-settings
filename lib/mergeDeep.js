class MergeDeep {
    ...
    for (const key in source) {
      ...
      if (this.isObject(source[key]) || Array.isArray(source[key])) {
        if (Array.isArray(source[key])) {
          // this.log.debug(`${JSON.stringify(t, null, 2)} ${JSON.stringify(s, null, 2)} ${JSON.stringify(additions, null, 2)} ${JSON.stringify(modifications, null, 2)}  `)
          // this.log.debug(`${key}  ${JSON.stringify(source[key], null, 2)} `)
          additions[key] = []
          modifications[key] = []
        } else {
          additions[key] = {}
  ...
  validateOverride (key, newValue, oldValue) {
    if (this.overridevalidators[key]) {
      // this.log.debug(`Calling overridevalidator for key ${key} `)
      if (!this.overridevalidators[key].canOverride(newValue, oldValue)) {
  ...
  validateConfig (key, currentValue) {
    if (this.configvalidators[key]) {
      // this.log.debug(`Calling configvalidator for key ${key} `)
      if (!this.configvalidators[key].isValid(currentValue)) {
            ...
            for (const a of temp) {
              if (this.isObject(a)) {
                if (visited[a.name]) {
                  ...
                  this.validateOverride(key, a, visited[a.name])
                  // this.log.debug(` Recursing to merge ${key} ${a.name} ${JSON.stringify(a, null, 2)} ${JSON.stringify(visited[a.name], null, 2)}`)
                  visited[a.name] = this.mergeDeep(a, visited[a.name])
                  continue
                } else if (visited[a.username]) {
                  this.validateOverride(key, a, visited[a.username])
                  visited[a.username] = this.mergeDeep(a, visited[a.username])
                  continue
                }
                if (a.name) {
                  visited[a.name] = a
                ...
                // If already seen this, don't add again
                visited[a] = visited[a] || a
              }
            }
            target[key] = []
            for (const fields of Object.keys(visited)) {
              target[key].push(visited[fields])
            }
            this.validateConfig(key, target[key])
          } else {
            this.validateOverride(key, target[key], source[key])
            // this.log.debug(` Recursing to merge key = ${key} target = ${JSON.stringify(target[key], null, 2)} source = ${JSON.stringify(source[key], null, 2)}`)
            target[key] = this.mergeDeep(target[key], source[key])
            // this.log.debug(` result of merge key = ${key} target = ${JSON.stringify(target[key], null, 2)} source = ${JSON.stringify(source[key], null, 2)}`)
            this.validateConfig(key, target[key])
          }
        } else {
