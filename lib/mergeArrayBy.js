...
function findMatchingIndex (properties, sourceItem, target) {
  const hasAnyProperty = item => properties.some(prop => Object.prototype.hasOwnProperty.call(sourceItem, prop))
  if (hasAnyProperty(sourceItem)) {
    return target
      .filter(targetItem => hasAnyProperty(targetItem))
      .findIndex(targetItem => properties.some(prop => sourceItem[prop] === targetItem[prop]))
  }
}
